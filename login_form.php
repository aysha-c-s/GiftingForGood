...
$pid=$row['pid'];
    $dbDate = $row['Expire_date']; // Date from MySQL in 'YYYY-MM-DD' format
    $currentDate = date('Y-m-d');
    $dbDateTime = new DateTime($dbDate);
    $currentDateTime = new DateTime($currentDate);

    // Compare dates
   if ($dbDateTime < $currentDateTime && $row['status']!="received") {
      $updateQuery = "UPDATE posts SET status='expired' WHERE pid='$pid'";
